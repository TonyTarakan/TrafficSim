...
TEST(LaneTransition, VehicleAdvancesToNextLaneOnRoute)
{
    ...
    auto route = engine.compute_route(0, 2);
...
TEST(LaneTransition, ForcedMergeClampsSublaneOnNarrowerLane)
{
    ...
    std::vector<Lane> lanes = {
        {.id = 0, .from = 0, .to = 1, .length = 100.f, .speed_limit = 20.f, .num_sublanes = 3},
        {.id = 1, .from = 1, .to = 2, .length = 100.f, .speed_limit = 20.f, .num_sublanes = 1},
    };
    engine.set_map(nodes, lanes);
...
TEST(LaneTransition, TwoStreamsBothReachSharedLane)
{
    ...
    SimEngine engine;
    std::vector<RoadNode> nodes = {
        {.id = 0, .pos = {.x = 0.f, .y = -30.f}},
        {.id = 1, .pos = {.x = 0.f, .y = 30.f}},
        {.id = 2, .pos = {.x = 150.f, .y = 0.f}},
        {.id = 3, .pos = {.x = 300.f, .y = 0.f}},
    };
    std::vector<Lane> lanes = {
    ...
    auto route_a = engine.compute_route(0, 3);
    auto route_b = engine.compute_route(1, 3);
    ...
    for (int tick = 0; tick < 2000; ++tick) {  // 40s
        ...
        for (const auto& v : engine.vehicles()) {
            if (v.lane_id == 2) ever_merged.insert(v.id);
        }
    }
