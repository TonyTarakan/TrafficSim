...
struct CrossroadsFixture {
    RoadNode junction{.id = NodeId{10}, .pos = {.x = 0.f, .y = 0.f}};
    RoadNode east_origin{.id = NodeId{100}, .pos = {.x = 50.f, .y = 0.f}};
    RoadNode north_origin{.id = NodeId{101}, .pos = {.x = 0.f, .y = -50.f}};
    RoadNode east_dest{.id = NodeId{102}, .pos = {.x = 100.f, .y = 0.f}};
    RoadNode north_dest{.id = NodeId{103}, .pos = {.x = 0.f, .y = -100.f}};
    RoadNode west_origin{.id = NodeId{104}, .pos = {.x = -50.f, .y = 0.f}};
    RoadNode south_origin{.id = NodeId{105}, .pos = {.x = 0.f, .y = 50.f}};

    Lane east_in{.id = LaneId{0}, .from = NodeId{100}, .to = NodeId{10}, .length = 50.f};
    Lane north_in{.id = LaneId{1}, .from = NodeId{101}, .to = NodeId{10}, .length = 50.f};
    Lane east_out{.id = LaneId{2}, .from = NodeId{10}, .to = NodeId{102}, .length = 50.f};
    Lane north_out{.id = LaneId{3}, .from = NodeId{10}, .to = NodeId{103}, .length = 50.f};
    Lane west_in{.id = LaneId{4}, .from = NodeId{104}, .to = NodeId{10}, .length = 50.f};
    Lane south_in{.id = LaneId{5}, .from = NodeId{105}, .to = NodeId{10}, .length = 50.f};
...
TEST(TrafficLightControl, NoPhasesIsAlwaysGreen)
{
    ...
    EXPECT_TRUE(light.is_green(LaneId{0}));
}
...
TEST(TrafficLightControl, RespectsCurrentPhase)
{
    TrafficLightControl light;
    light.phases = {
        {.green_lanes = {LaneId{0}}, .duration = 10.f},
        {.green_lanes = {LaneId{1}}, .duration = 10.f},
    };

    EXPECT_TRUE(light.is_green(LaneId{0}));
    EXPECT_FALSE(light.is_green(LaneId{1}));
}
...
TEST(TrafficLightControl, AdvanceSignalsCyclesPhases)
{
    ...
    EXPECT_TRUE(light->is_green(LaneId{1}));
    EXPECT_FALSE(light->is_green(LaneId{0}));
}
...
TEST(JunctionMap, FindByNodeAndLane)
{
    ...
    EXPECT_EQ(map.find_by_lane(LaneId{0})->node_id, NodeId{10});
    EXPECT_EQ(map.find_by_lane(LaneId{1})->node_id, NodeId{10});
    EXPECT_EQ(map.find_by_node(NodeId{999}), nullptr);
    EXPECT_EQ(map.find_by_lane(LaneId{999}), nullptr);
}

TEST(JunctionMap, RebuildResolvesGeometryFromNodesAndLanes)
{
    CrossroadsFixture f;
    Junction j{.node_id = NodeId{10}, .incoming = {LaneId{0}, LaneId{1}}};
...
{
    Junction j{.node_id = NodeId{999}, .incoming = {LaneId{123}}};  // nothing in the (empty) map matches
...
TEST(FindJunctionYield, PriorityLaneWithNoRivalsProceeds)
{
    ...
    Junction j{.node_id = NodeId{10},
               .incoming = {LaneId{0}, LaneId{1}},
               .control = PriorityControl{.yields_to = {{LaneId{1}, {LaneId{0}}}}}};  // lane 1 yields to lane 0
    JunctionMap map;
    map.rebuild({j}, f.all_nodes(), f.all_lanes());

    // Ego is on lane 0, the main road -- not in anyone's yields_to list.
    Vehicle ego{.id = VehicleId{0}, .lane_id = LaneId{0}, .offset = 45.f};
...
TEST(FindJunctionYield, MinorLaneYieldsToCloseRival)
{
    ...
    Junction j{.node_id = NodeId{10},
               .incoming = {LaneId{0}, LaneId{1}},
               .control = PriorityControl{.yields_to = {{LaneId{1}, {LaneId{0}}}}}};
    JunctionMap map;
    map.rebuild({j}, f.all_nodes(), f.all_lanes());

    Vehicle ego{.id = VehicleId{1}, .lane_id = LaneId{1}, .offset = 45.f};  // on the minor approach, 5m from the line
    Vehicle rival{.id = VehicleId{0}, .speed = 10.f, .lane_id = LaneId{0}, .offset = 30.f};  // 20m out, 2s to the line
...
TEST(FindJunctionYield, MinorLaneProceedsWhenGapIsWideEnough)
{
    ...
    Vehicle ego{.id = VehicleId{1}, .lane_id = LaneId{1}, .offset = 45.f};
    // Rival is 40m out doing 5 m/s -- 8s to the line, comfortably over the 4s gap threshold.
    Vehicle rival{.id = VehicleId{0}, .speed = 5.f, .lane_id = LaneId{0}, .offset = 10.f};
...
TEST(FindJunctionYield, YieldsWhileJunctionBoxIsOccupied)
{
    ...
    Vehicle crossing{.id = VehicleId{2}, .lane_id = LaneId{2}, .offset = 3.f};
...
TEST(FindJunctionYield, RedLightForcesStop)
{
    ...
    Vehicle ego{.id = VehicleId{1}, .lane_id = LaneId{1}, .offset = 45.f};  // lane 1 is red in phase 0
...
TEST(FindJunctionYield, GreenLightProceeds)
{
    ...
    Vehicle ego{.id = VehicleId{0}, .lane_id = LaneId{0}, .offset = 45.f};  // lane 0 is green in phase 0
    auto result = leader_to_yield(ego, f.east_in, map, {}, f.all_lanes());
    EXPECT_FALSE(result.has_value());
}
...
TEST(FindJunctionYield, UnregulatedYieldsToTrafficOnTheRight)
{
    CrossroadsFixture f;
    Junction j{.node_id = NodeId{10},
               .incoming = {LaneId{0}, LaneId{1}, LaneId{4}, LaneId{5}},
    ...
    Vehicle from_north{.id = VehicleId{1}, .speed = 5.f, .lane_id = LaneId{1}, .offset = 35.f};
...
TEST(FindJunctionYield, UnregulatedDoesNotYieldToTrafficOnTheLeft)
{
    ...
    Vehicle ego{.id = VehicleId{0}, .lane_id = LaneId{0}, .offset = 45.f};
    Vehicle from_south{.id = VehicleId{1}, .speed = 5.f, .lane_id = LaneId{5}, .offset = 49.f};
...
TEST(FindJunctionYield, UnregulatedDoesNotYieldToOncomingTraffic)
{
    ...
    Vehicle ego{.id = VehicleId{0}, .lane_id = LaneId{0}, .offset = 45.f};
    Vehicle oncoming{.id = VehicleId{1}, .speed = 5.f, .lane_id = LaneId{4}, .offset = 45.f};
...
TEST(FindJunctionYield, UnregulatedIsNonReciprocal)
{
    ...
    Vehicle from_north{.id = VehicleId{1}, .lane_id = LaneId{1}, .offset = 45.f};
    Vehicle ego{.id = VehicleId{0}, .speed = 5.f, .lane_id = LaneId{0}, .offset = 20.f};
